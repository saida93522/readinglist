...
class Book:
    ...
    def save(self):
        ...
        When trying to add a new book, first book title and author name are provided by user, then Save function is called. 
        If Save function detects that book title and author name already exists and find book id, then the update_book method will be called
        to update book info that has been modified (only read status for now); otherwise add_book method will be called to save book on DB.
        """
        if self.id:
...
class BookStore:
    ...
    class __BookStore:
        ...
        def _add_book(self, book):
            ...
            try:
                with sqlite3.connect(db) as con:
                    res = con.execute(
                        insert_sql, (book.title, book.author, book.read))
                    new_id = res.lastrowid  # Get the ID of the new row in the table
                    book.id = new_id  # Set this book's ID
            except sqlite3.IntegrityError as e:
                raise BookError(
                    f'Error - this book is already in the database. {book}') from e
            finally:
                con.close()
        ...
        def exact_match(self, search_book):
            ...
            con = sqlite3.connect(db)
            rows = con.execute(find_exact_match_sql,
                               (search_book.title, search_book.author))
        ...
        def book_search(self, term):
            ...
            # Example - if searching for text with 'bOb' in then use '%bOb%' in SQL
            search = f'%{term}%'
        ...
        def book_count(self):
            ...
            # fetchone() returns the first row of the results. This is a tuple with one element - the count
